...
b <- des |> filter(!is.na(harsh),!is.na(aestudio)) |> group_by(harsh) |>
  ...
  filter(!is.na(harsh)) |>
  mutate(grp=ifelse(harsh==1,"Harsh-discipline\nhome","Other home"),
         hi = est + 1.96*est_se)
b$grp <- factor(b$grp, levels=c("Other home","Harsh-discipline\nhome"))
...
pB <- ggplot(b, aes(grp,est,fill=grp))+geom_col(width=0.6)+
  ...
  geom_text(aes(y=hi+0.4,label=sprintf("%.1f yrs",est)),vjust=0,fontface="bold",color=ink,size=5)+
  scale_fill_manual(values=c("Harsh-discipline\nhome"=pink,"Other home"=blue))+
  scale_y_continuous(limits=c(0,13),expand=expansion(mult=c(0,0.05)))+
  labs(title="Years of schooling by household discipline",
       subtitle="Women 12–49 · EDSA 2023, survey-weighted", x=NULL,y=NULL)+base

ggsave(here::here("output","figures","f7b_diagnosis_panel_en.png"), pB, width=6.4, height=4.8, dpi=150, bg="white")
cat("wrote f7b_diagnosis_panel_en.png (single panel)\n")
