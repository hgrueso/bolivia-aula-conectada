...
f8 <- ggplot(vi_plot, aes(x = outcome_label, y = var_label, fill = importance)) +
  ...
  scale_fill_gradient(low = "#9DD4EC", high = UNICEF_DARK,
...
save_fig(f8, "f8_var_importance", width = 10, height = 5)
...
f9 <- ggplot(girl_preds,
  ...
  scale_fill_manual(values = c("Asiste a la escuela"    = UNICEF_DARK,
                               "Internet en hogar"      = UNICEF_BLUE,
                               "Cualquier dispositivo"  = "#9DD4EC",
                               "Computadora en hogar"   = ACCENT_GIRL)) +
  labs(
